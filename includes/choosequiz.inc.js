  ...
  setQuizze(error, data) {
    return new Promise((resolve, reject) => {
      ...
      if (error) {
        ...
        this.quizzeContainer.innerHTML = `
                    <p class="quizAuswahlDescription"><b>Quiz dazu gefunden</b> (${sorted.length})</p>
                    <ul class="results">
...
function searchBar() {
  ...
  let showresultQuize = (resutls) => {
    ...
    if (!resutls) {
      quizResultContainer.innerHTML = `
            <h5>Quiz <span>(0)</span></h5>
            <p>Keine Quiz dazu gefunden...<p>
            `;
      return;
    }
    ...
    let quizResultContainerInner = `
        <h5>Quiz <span>(${sorted.length})</span></h5>
                        <ul class="quize-ul">
                        ${links}
                        </ul>
        `;
